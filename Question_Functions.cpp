...
int Template::InitQuestion(){
    ...
    surface = IMG_Load("pic.jpg");
            //if the picture did not load, set the surface to a solid color, so background_tex will not be NULL
    
    if(surface == NULL){
        SDL_LogError(SDL_LOG_CATEGORY_VIDEO, "Couldn't load image: %s", SDL_GetError());
        surface = SDL_CreateRGBSurface(0, WINDOW_LENGTH, WINDOW_HEIGHT, 32, 0, 0, 0, 0);
    }
    ...
    SDL_FreeSurface(surface);
               ...
               //set locations for answer box highlights
    SDL_Rect temp_rect;
   ...
   printf("In question function.\n");
    ...
    SDL_RenderClear(renderer);
        ...
        if (skip == true) {
             ...
             RenderText("skipped", 32, 100, 100, NULL, 0, NULL, flip);
        } else if (answer == correct) {
            printf("Answer correct!\n");
            RenderText("CORRECT!",32, 100, 100, NULL, 0, NULL, flip);
        } else {
            printf("Wrong!\n");
            RenderText("WRONG!",32, 100, 100, NULL, 0, NULL, flip);
        }
...
void Template::DisplayQuestion ( ) {
    ...
    //render answer box highlight, this changes when user changes answer selection.
...
void Template::ResetQuestion(){
    SDL_RenderClear(renderer);
    answer = 0;    // this is selected answer - different than correct answer
    nextQ = false;
    skip = false;
    checkAnswer = false;
}
